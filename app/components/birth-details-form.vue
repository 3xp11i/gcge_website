<template>
	<UForm
		...
		<div class="grid gap-5 sm:grid-cols-2" v-show="formStep === 'birthDetails'">
			<UFormField
			...
			<UFormField label="Date of birth" name="dateOfBirth">
				<UInputDate ref="inputDate" required v-model="state.dateOfBirth">
					<template #trailing>
			...
			<UFormField label="Birth Location" name="birthLocation" class="">
				<UInput
			...
			<UFormField label="Birth Place Zipcode" name="zipcode" class="">
				<UInput
			...
			<UFormField
				label="How would you like your consultation?"
				name="consultationMethod"
				class="sm:col-span-2 w-full">
				<USelectMenu
					v-model="state.consultationMethod"
					:items="
						Object.entries(consultationMethodOptions).map(([value, label]) => ({
							value,
							label,
						}))
					"
			...
			<UFormField
				v-if="state.consultationMethod === 'instagram'"
			...
			<UFormField
				name="needsBtr"
				label="Choose birth time accuracy level"
				class="sm:col-span-2 w-full">
		...
		<div v-show="formStep === 'slotBooking'" class="space-y-5">
			...
			<div
				...
				<button
					type="button"
					class="underline"
					@click="fetchMonthSlots(calendarPage)">
					Retry
				</button>
			</div>
			<div v-if="slotConflictError" class="text-sm text-red-400 pt-1">
				{{ slotConflictError }}
			</div>
			...
			<div class="flex flex-col gap-6 sm:flex-row sm:items-start">
				...
				<div class="shrink-0">
					<div
						v-if="
							availabilityStatus === 'idle' || availabilityStatus === 'pending'
						"
						class="space-y-3">
				...
				<div class="flex-1 min-w-40 overflow-y-auto max-h-52 sm:max-h-[22rem]">
					...
					<template v-else>
						<div v-if="!selectedDate" class="text-sm text-white/40 pt-1">
						...
						<div v-else class="space-y-4">
							<div
								v-for="period in slotsByPeriod"
								:key="period.key"
								class="space-y-2">
								<span
									class="text-lg font-semibold uppercase tracking-wider text-white/50"
									>{{ period.label }}</span
								>
								<div
									class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-2 pr-4">
									<button
		...
		<div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
			...
			<UButton
				type="button"
				v-if="formStep === 'slotBooking' && selectedSlot && !slotConfirmed"
				class="button justify-center"
			...
			<UButton
				type="submit"
				v-if="formStep === 'slotBooking' && slotConfirmed"
				class="button justify-center"
				:loading="loading">
				{{ submitLabel || "Submit" }}
			</UButton>
			<UButton
				...
				@click="
					() => {
						formStep = 'birthDetails';
						$emit('slotBookingFormActive', false);
					}
				"
...
const slotConflictError = ref<string | null>(null);
const confirmingSlot = ref(false);
const toast = useToast();

async function confirmSlot() {
	if (!selectedSlot.value) return;
	slotConflictError.value = null;
	confirmingSlot.value = true;
	try {
		const result = await $fetch<{ available: boolean }>("/api/check-slot", {
			method: "POST",
			body: {
				starts_at: selectedSlot.value.start.toISOString(),
				ends_at: selectedSlot.value.end.toISOString(),
			},
		});
		if (result.available) {
			...
			toast.add({
				...
				description: `Your slot at ${formatSlotTime(selectedSlot.value.start)} is available. Proceed to payment.`,
				color: "success",
				icon: "i-lucide-circle-check",
			});
		} else {
			slotConflictError.value =
				"This slot was just booked. Please choose another time.";
			toast.add({
				title: "Slot no longer available",
				description: "Sorry, that slot was just taken. Please choose another time.",
				color: "error",
				icon: "i-lucide-circle-x",
			});
			// Remove conflicting slot from local candidates
			availability.value = availability.value.map((day) => ({
				...day,
				bookings: [
					...day.bookings,
					{
		...
		slotConflictError.value =
			"Failed to verify slot availability. Please try again.";
		toast.add({
			title: "Verification failed",
			description: "Could not verify slot availability. Please try again.",
			color: "error",
...
const serviceDurationMinutes = computed(
	() => props.serviceDurationMinutes ?? 30,
);

const availability = ref<DayAvailability[]>([]);
const availabilityStatus = ref<"idle" | "pending" | "success" | "error">(
	"idle",
);
...
const fetchMonthSlots = async (page: CalendarDate) => {
	...
	if (page.compare(new CalendarDate(maxPage.year, maxPage.month, 1)) >= 0)
		return;
	...
	try {
		const data = await $fetch<DayAvailability[]>("/api/available-slots", {
...
const calendarMaxDate = today(getLocalTimeZone())
	.add({ months: 4 })
	.subtract({ days: 1 });
...
const {
	...
	status,
	execute,
} = await useLazyFetch<PhoneCode[]>("/api/phone-codes", {
	key: "api-phone-codes",
	immediate: false,
});
...
const state = reactive<BirthDetails>({
	...
	dateOfBirth: shallowRef(
		new CalendarDate(n.getFullYear(), n.getMonth(), n.getDate()),
	),
	timeOfBirth: shallowRef(
		new Time(n.getHours(), n.getMinutes(), n.getSeconds()),
	),
	location: "",
	zipcode: "",
...
const validate = (formState: Partial<BirthDetails>): FormError[] => {
	...
	if (!formState.consultationMethod) {
		errors.push({
			name: "consultationMethod",
			message: "Choose how you want your consultation.",
		});
	}

	if (
		formState.consultationMethod === "instagram" &&
		!formState.instagramUsername
	) {
		errors.push({
			name: "instagramUsername",
			message: "Instagram username is required for Instagram consultations.",
		});
	}
